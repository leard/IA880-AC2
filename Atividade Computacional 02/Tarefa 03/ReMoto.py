...
def set_stimulation_page(drive,
    ...
    time.sleep(1.0)
    # stimulation_intensity = drive.find_element_by_xpath(
    #     '/html/body/div[2]/div[2]/form/table/tbody/tr[2]/td/table/tbody/tr[4]/td/table/tbody/tr/td[5]/input')
    # stimulation_intensity.get_attribute('value')
    # Ajustando tempo
    if cpn:
...
parameterization_h_reflex_cond(reMoto,
                               control=False,
                               s=[20.0, 14.0],
                               fr=[14.0, 13.5],
                               ff=[13.5, 13.0],
...
# # Neural Elements
# set_neural_elements(reMoto)
# time.sleep(2.0)
# # Motoneurons
# set_motoneuron_config(reMoto, s=[20.0, 14.0], fr=[14.0, 13.5], ff=[13.5, 13.0])
# time.sleep(2.0)
# # Sensory Fibers
# set_sensory_fibers_config(reMoto, ia_axon_thr=[9.0, 18.0])
# time.sleep(2.0)
########################################################################################################################
# Simulação
########################################################################################################################
#
intervalos_cpn = [2.0, 3.0]
estimulo_cpn_values = 15.00

estimulo_ptn_values = 10.70


#experimentos_folder = ["./ctl"+str(i)+"/" for i in range(1, 21)]

experimentos_folder = ["./exp"+str(i)+"/" for i in range(19, 21)]

#set_stimulation_page(drive=reMoto, estimulo_ptn=estimulo_ptn_values, intervalo_ptn=[0, 1])
...
for experimento in experimentos_folder:
    ...
    # intervalos_ptn_ = zip([0], [1])
    intervalos_ptn_ = zip(np.append(np.append(arange(0.0, 15.00, 0.5), arange(15.0, 25.00, 1.0)),
                                    arange(25.0, 50.00, 2.0)),
                          np.append(np.append(arange(1.0, 16.00, 0.5), arange(16.0, 26.00, 1.0)),
                                    arange(26.0, 51.00, 2.0)))
    for intervalo_ptn in intervalos_ptn_:
        print(f'Simulation for {intervalo_ptn[0]:.2f} ms')
        set_stimulation_page(drive=reMoto,
                             estimulo_ptn=estimulo_ptn_values,
                             intervalo_ptn=intervalo_ptn,
                             intervalo_cpn=intervalos_cpn,
                             estimulo_cpn=estimulo_cpn_values,
                             cpn=True)
